...
function HeroSection({ heading, paragraph, btn1, btn2, img, redirect1, redirect2 }) {
  ...
  const handleButtonClick1 = () => {
    ...
    if (!userInfo._id && redirect1 === "call") {
      navigate('/login');
      return;
    }
  ...
  const handleButtonClick2 = () => {
    if (!userInfo._id && redirect2 === "joinCall") {
      navigate('/login');
      return;
    }
    navigate(`/${redirect2}`);
      ...
      <div className=" pr-4 overflow-x-hidden flex-between flex-col lg:flex-row">
        <div className=" w-full lg:w-[38%] flex items-start justify-center flex-col ml-4 lg:ml-20 lg:items-start lg:justify-start">
          ...
          <div className="flex-between">
            <Button loading={meetLoading} text={btn1} onClick={handleButtonClick1} className="text-white px-6 py-4 bg-primary rounded-2xl hover:shadow-secondary shadow-sm transition-all duration-300 ease-in-out mr-3" />
